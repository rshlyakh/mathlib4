...
variable {k A G : Type*} [Semiring k]

@[to_additive (dont_translate := k) (attr := simp)
  AddMonoidAlgebra.single_mem_iff_of_isUnit, simp]
lemma MonoidAlgebra.single_mem_iff_of_isUnit [Monoid G]
    (I : Ideal (MonoidAlgebra k G)) (a : G) {r : k} (hr : IsUnit r) :
    single a r ∈ I ↔ single a 1 ∈ I := by
  calc
    single a r ∈ I ↔ single 1 r * single a 1 ∈ I := by
      simp
    _ ↔ single a 1 ∈ I :=
      I.unit_mul_mem_iff_mem (hr.map MonoidAlgebra.singleOneRingHom)

@[to_additive (dont_translate := k)]
lemma MonoidAlgebra.mem_of_forall_single_one_mem [Monoid G] (I : Ideal (MonoidAlgebra k G))
    {f : MonoidAlgebra k G}
    (h : ∀ a ∈ f.coeff.support, single a 1 ∈ I) : f ∈ I := by
  rw [← MonoidAlgebra.sum_coeff_single f]
  apply I.sum_mem
  intro a ha
  simpa using I.mul_mem_left (MonoidAlgebra.single 1 (f.coeff a)) (h a ha)
...
-/
theorem MonoidAlgebra.mem_ideal_span_of_image [Monoid G] {s : Set G}
    {x : MonoidAlgebra k G} :
...
-/
theorem AddMonoidAlgebra.mem_ideal_span_of'_image [AddMonoid A] {s : Set A}
